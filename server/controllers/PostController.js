...
exports.singleRequest = async (req, res) => {
    ...
    await Formpost.findOne({ slug }).then(async (response) => {
        if (!response) {
...
exports.deleteRequest = async (req, res) => {
    const { slug } = req.params
    await Formpost.findOneAndDelete({ slug }).then((post) => {
        if (!post) {
            return res.status(404).json({ message: 'ไม่พบบัญชีนี้' })
        }
...
exports.updateComfirmed = async (req, res) => {
    const { slug } = req.params
    await Formpost.findOneAndUpdate({ slug }, { isConfirmed: true }, { new: true })
        .then((response) => {
            res.status(200).json({ response })
        }).catch((err) => {
